...
import { View, Text, Image, TouchableOpacity, Dimensions, Animated, SafeAreaView } from "react-native";
...
const ImageDetailsScreen = () => {
  type ImageDetailsRouteParams = {
    params: {
      ...
      name: string;
      path?: string;
      imageData: { file_path: string; file_path_heatmap: string; confidence_score: number };
    };
  };
  ...
  return (
    <SafeAreaView className="flex-1 bg-backgroundPrimary">
      ...
      <TouchableOpacity onPress={() => navigation.goBack()} className="pr-3">
        ...
        <Text style={{ fontFamily: 'ClimateCrisis', fontSize: 32, color: '#21FA90' }}>
          Details
        </Text>
      ...
      <View className="flex-1">
          <Animated.View style={{ opacity: fadeAnim, transform: [{ translateY: slideAnim }], marginTop: 20, }}>
            <ViewShot ref={viewShotRef} options={{ format: "png", quality: 0.9 }}>
              <View
                className={`flex flex-col items-center justify-between p-4 rounded-lg ${
                  aiPercent > 0.5 ? "bg-fontColorPrimary" : "bg-fontColorSecondary"
                }`}
              >
                  ...
                  {aiPercent !== null && phrase && (
                    <TouchableOpacity
                      ...
                      className={`p-3 rounded-lg flex-row justify-center items-center ${
                        aiPercent > 0.5 ? "bg-backupColor2" : "bg-fontColorPrimary"
                      }`}
                    >
                ...
                <View className="flex flex-row items-center mt-1">
                  {aiPercent !== null && phrase && (
                    <Donut AI={(aiPercent * 100).toFixed(2)} notAI={(100 - aiPercent * 100).toFixed(2)} />
                  )}
                </View>
              </View>
            </ViewShot>
